...
brca_clin %>% filter(!is.na(times)) %>%
  ggplot(aes(x=times)) +
    ...
    geom_vline(aes(xintercept=mean(times)), color="blue", linetype="dashed", size=1) +
    labs(title="Time to Event")

brca_clin %>% dplyr::select(age) %>% summary()

###Censoring and Event Plots  by Age Category
The TCGA clinical data set is a survival dataset containing  right censor information. We will first visualize the distribution of teh right censor data by different categories.
, 
...
paste("Constant Hazard Rate for Exponential Distribution without any predictor:",format(lambda,2), " events per year")
...
#lambda <- exp(-1.0 * unname(surv_exp_age$icoef[1]))
...
paste("Constant Hazard Rate for Exponential Distribution with therapy as predictor:",format(lambda, digits=2) , "events/year")
...
The survival data is fit with  parametric models using both the exponential and weibull distributions and compared.
The fitness of the each of these models is verified by checking the loglikelihood values of the models. For both the models,  p-values for the chi-square distribution with one degree of freedom are high (> 0.05) indicating that botht he parametric models are not good fits. 
...
**Check for Proportional Hazards Assumptoin**
```{r}
