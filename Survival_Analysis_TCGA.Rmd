...
ggplot_can_surv <- ggsurvplot(surv_can_fit,
           pval = TRUE,
           conf.int = TRUE,
...
ggplot_can_cumhaz <- ggsurvplot(surv_can_fit,
           pval = TRUE,
           conf.int = TRUE,
           conf.int.fill="strata",
           conf.int.alpha=0.2,
...
arrange_ggsurvplots(list(ggplot_can_surv, ggplot_can_cumhaz), ncol=1)
```

### Check for Difference between the survival curves
```{r}
survdiff(surv_can_obj ~ factor(admin.disease_code,exclude=NULL),data=clin)
surv_pairdiff_can <- pairwise_survdiff(Surv(years_to_event,patient.vital_status)~admin.disease_code,data=clin)
surv_pairdiff_can
```
...
The fitness of each of these models is verified by checking the loglikelihood values of the models. For both the models,  p-values for the chi-square distribution with one degree of freedom are very small (<0.05) indicating that models are good fits. 
...
The survival data is fit with  parametric models using both the exponential and weibull distributions and compared.
The fitness of each of these models is verified by checking the loglikelihood values of the models. For both the models,  p-values for the chi-square distribution with one degree of freedom are high (> 0.05) indicating that models are not good fits. 
...
When the survival curves for the different tumor stages are compared, the difference in the curves is not found to be significantly different.
...
The fitness of each of these models is first verified by checking the loglikelihood values of the models. For both the models,  p-values for the chi-square distribution with one degree of freedom are high (> 0.05) indicating that both the parametric models are not good fits. 
...
The Cox model equation can then be written as: 
 $$  H(t|age,,no_info, stage3,stage4,stageX) = 
...
1. For every increase in age at diagnosis by one year, the risk of death increases by a factor of 1.02.
2. Compared to subjects on chemotherapy treatment, the risk of death for subjects with No info on the therapy increases by a factor of 3.25
3. Compared to subjects diagnosed with stage 1 cancer, the risk of death increases by a factor of 2.98 for subjects diagnosed with cancer stage 3  
4. Compared to subjects diagnosed with stage 1 cancer, the risk of death increases by a factor of 5.39 for subjects diagnosed with cancer stage 4  
5. Compared to subjects diagnosed with stage 1 cancer, the risk of death increases by a factor of 2.64 for subjects diagnosed with cancer stage X (or not assessed) cancer 


### Cox model with Age, Therapy, Tumor, and Lymph Node Stages
```{r}
cox_all_2 <- coxph(surv_obj_all ~ age + therapy_type +  pathologyNstage, data=brca_clin)
...
The Cox model equation can then be written as: 
 $$  H(t|age,no_info,n2,n3,nX) = 
H_0(t) exp(0.02*age + 1.35*No\_info + 0.55*n1 + 1.47*n2 + 1.65*n3 + 0.86*stageX) 
$$ 
This equation can be used to compute the cumulative hazard rate for new values. 
...
1. For every increase in age at diagnosis by one year, the risk of death increases by a factor of 1.02.
2. Compared to subjects on chemotherapy treatment, the risk of death for subjects with No info on the therapy increases by a factor of 3.96
3.  Compared to subjects diagnosed with nstage 0 cancer, the risk of death increases by a factor of 1.73 for subjects diagnosed with stage n1
4. Compared to subjects diagnosed with nstage 0 cancer, the risk of death increases by a factor of 4.3 for subjects diagnosed with stage n2
5. Compared to subjects diagnosed with nstage 0 cancer, the risk of death increases by a factor of 5.2 for subjects diagnosed with stage n3
6. Compared to subjects diagnosed with nstage 0 cancer, the risk of death increases by a factor of 2.4 for subjects diagnosed with stage nX (or not assessed) cancer 
...
5. Therapy Type:
      a. Kaplan Meir Results: "No Info" group is significantly different from the other therapy groups with the survival of probability descreasing rapidly with no treatment.
...
10. Combined:
      ...
      b. Cox Proportional Hazards Model (Age, Therapy, Lymph Nodes Stage): The base hazard model corresponds to age=0, treatment type of chemotherapy, tumor stage 1 and lymph node stage 0 (n0 - no cancer in lymph nodes). Of the other predictors, the coefficients for  age, therapy_type of "No info", lymph node stages n2,n3, and nX are found to be significant.
         ...
         i. For every one additional year of age, the risk of death increases by a factor of 1.02.
         ii. Compared to subjects on chemotherapy treatment, the risk of death for subjects with No info on the therapy increases by a factor of 3.86
         iii. Compared to subjects diagnosed with nstage 0 cancer, the risk of death increases by a factor of 1.7 for subjects diagnosed with stage n1
         iii. Compared to subjects diagnosed with nstage 0 cancer, the risk of death increases by a factor of 4.3 for subjects diagnosed with stage n2
         iv. Compared to subjects diagnosed with nstage 0 cancer, the risk of death increases by a factor of 5.2 for subjects diagnosed with stage n3
         v. Compared to subjects diagnosed with nstage 0 cancer, the risk of death increases by a factor of 2.4 for subjects diagnosed with stage nX (or not assessed) cancer  
